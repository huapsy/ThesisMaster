...
def _preconfigure_mplconfigdir() -> None:
    configured = str(os.environ.get("MPLCONFIGDIR", "")).strip()
    candidates = []
    if configured:
    ...
    candidates.extend(
        [
            Path.cwd() / ".mplconfig",
            Path(tempfile.gettempdir()) / "phoenix_mplconfig",
        ]
    )
...
def parse_args() -> argparse.Namespace:
    ...
    default_run_root = repo_root / "evaluation/integrated_pipeline/runs"
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Optional report output path. Defaults to <run-root>/10_research_reports.",
    )
...
def _collect_profiles(
    ...
    def resolve_stage_root(primary_rel: str, legacy_rel: str) -> Path:
        primary = run_root / primary_rel
        if primary.exists():
            return primary
        return run_root / legacy_rel

    readiness_root = resolve_stage_root("03_readiness_check", "00_readiness_check")
    network_root = resolve_stage_root("04_time_series_analysis/network", "01_time_series_analysis/network")
    impact_root = resolve_stage_root("05_momentary_impact_coefficients", "02_momentary_impact_coefficients")
    handoff_root = resolve_stage_root("06_target_identification_and_model_update", "03_treatment_target_handoff")
    intervention_root = resolve_stage_root("07_hapa_digital_intervention", "03b_translation_digital_intervention")
...
def main() -> int:
    ...
    output_root = (
        ...
        else (run_root / "10_research_reports").resolve()
    )
