...
def test_hapa_component_candidates_are_grouped_by_subtree(module_loader, repo_file_fn) -> None:
    module = _mod(module_loader, repo_file_fn)
    payload = module.build_hapa_component_candidates(
        predictor_candidates=[
            {
                "predictor": "P01",
                "predictor_label": "Sleep regularity",
        ...
        selected_barriers=[
            {
                "barrier_name": "Risk_Perception",
                "barrier_path": "BARRIERS / Motivation / Risk_Perception",
                "barrier_parent_domain": "BARRIERS / Motivation",
                "total_score_0_1": 0.66,
                "supporting_predictors": ["BIO / Sleep / Sleep_Regularity / Fixed_Bedtime"],
            },
            {
                "barrier_name": "Planning",
                "barrier_path": "BARRIERS / Volition / Planning",
                "barrier_parent_domain": "BARRIERS / Volition",
                "total_score_0_1": 0.64,
                "supporting_predictors": ["PSYCHO / Activation / Behavioral_Activation / Activity_Scheduling"],
            },
        ],
        coping_candidates=[
            {
        ...
        barrier_detail_rows=[
            {
                "barrier_name": "Planning",
                "barrier_path": "BARRIERS / Volition / Planning",
                "predictor_barrier_contribution_0_1": 0.61,
            }
        ],
    )
    assert isinstance(payload, dict)
    assert payload.get("component_order")
    components = payload.get("components") or []
    assert components
    motivation = next((row for row in components if row.get("component") == "Motivation"), None)
    planning = next((row for row in components if row.get("component") == "Action and Coping Planning"), None)
    assert motivation is not None
    assert planning is not None
    assert (motivation.get("subtree_nodes") or [])
    assert (planning.get("barriers") or [])
