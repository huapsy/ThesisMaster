...
def build_iterative_cycle_input_root(
    *,
    ...
    for profile_id in profiles:
        ...
        handoff_roots = [
            source_cycle_root / "06_target_identification_and_model_update",
            source_cycle_root / "03_treatment_target_handoff",
        ]
        intervention_roots = [
            source_cycle_root / "07_hapa_digital_intervention",
            source_cycle_root / "03b_translation_digital_intervention",
        ]
        readiness_roots = [
            source_cycle_root / "03_readiness_check",
            source_cycle_root / "00_readiness_check",
        ]
        impact_roots = [
            source_cycle_root / "05_momentary_impact_coefficients",
            source_cycle_root / "02_momentary_impact_coefficients",
        ]

        step04_path = next(
            (
                root / profile_id / "step04_updated_observation_model.json"
                for root in handoff_roots
                if (root / profile_id / "step04_updated_observation_model.json").exists()
            ),
            handoff_roots[0] / profile_id / "step04_updated_observation_model.json",
        )
        step03_bundle_path = next(
            (
                root / profile_id / "step03_evidence_bundle.json"
                for root in handoff_roots
                if (root / profile_id / "step03_evidence_bundle.json").exists()
            ),
            handoff_roots[0] / profile_id / "step03_evidence_bundle.json",
        )
        step03_target_path = next(
            (
                root / profile_id / "step03_target_selection.json"
                for root in handoff_roots
                if (root / profile_id / "step03_target_selection.json").exists()
            ),
            handoff_roots[0] / profile_id / "step03_target_selection.json",
        )
        step05_path = next(
            (
                root / profile_id / "step05_hapa_intervention.json"
                for root in intervention_roots
                if (root / profile_id / "step05_hapa_intervention.json").exists()
            ),
            intervention_roots[0] / profile_id / "step05_hapa_intervention.json",
        )
        readiness_path = next(
            (
                root / profile_id / "readiness_report.json"
                for root in readiness_roots
                if (root / profile_id / "readiness_report.json").exists()
            ),
            readiness_roots[0] / profile_id / "readiness_report.json",
        )
        impact_csv_path = next(
            (
                root / profile_id / "predictor_composite.csv"
                for root in impact_roots
                if (root / profile_id / "predictor_composite.csv").exists()
            ),
            impact_roots[0] / profile_id / "predictor_composite.csv",
        )
