...
def _find_repo_root() -> Path:
    ...
    for candidate in [here, *here.parents]:
        ...
        if has_eval and (candidate / "README.md").exists():
            return candidate
    raise RuntimeError("Could not locate repository root from apply_multi_rank_fusion.py")
...
INITIAL_MODEL_ASSETS_ROOT = REPO_ROOT / "src/backend/utils/agentic_core/others/initial_observation_model_assets"
HYDE_RUNS_ROOT = INITIAL_MODEL_ASSETS_ROOT / "helpers/00_HyDe_based_predictor_ranks/runs"


def _default_hyde_dense_profiles_csv() -> str:
    candidates = sorted(HYDE_RUNS_ROOT.glob("*/dense_profiles.csv"))
    if candidates:
        return str(candidates[-1])
    return str(HYDE_RUNS_ROOT / "2026-01-15_19-51-58/dense_profiles.csv")


DEFAULT_MAPPING_RANKS_CSV = str(
...
DEFAULT_HYDE_DENSE_PROFILES_CSV = _default_hyde_dense_profiles_csv()
DEFAULT_HYDE_PROFILES_DIR = str(Path(DEFAULT_HYDE_DENSE_PROFILES_CSV).parent / "profiles")

DEFAULT_OUTPUT_DIR = str(
    INITIAL_MODEL_ASSETS_ROOT / "helpers/01_fused_ranks/results"
)
