...
def _find_repo_root() -> Path:
    ...
    for candidate in [here, *here.parents]:
        ...
        if has_eval and (candidate / "README.md").exists():
            return candidate
    raise RuntimeError("Could not locate repository root from aggregate_predictor_jsons.py")
...
INPUT_DIR = str(
    REPO_ROOT
    / "src/backend/SystemComponents/PHOENIX_ontology/separate/01_raw/PREDICTOR/steps/01_raw/separate"
)

OUTPUT_DIR = str(
    REPO_ROOT
    / "src/backend/SystemComponents/PHOENIX_ontology/separate/01_raw/PREDICTOR/steps/01_raw/aggregated"
)
