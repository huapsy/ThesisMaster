...
def _find_repo_root() -> Path:
    ...
    for candidate in [here, *here.parents]:
        ...
        if has_eval and (candidate / "README.md").exists():
            return candidate
    raise RuntimeError("Could not locate repository root from 02_operationalize_freetext_multiple_issues_with_interface.py")
...
input_file = str(
    REPO_ROOT
    / "src/backend/SystemComponents/PHOENIX_ontology/separate/01_raw/CRITERION/steps/01_raw/aggregated/CRITERION_ontology.json"
)
...
DEFAULT_OUTPUT_DIR = str(
    REPO_ROOT
    / "src/backend/SystemComponents/Agentic_Framework/01_OperationalizationMentalHealthProblem/tmp"
)
