...
DEFAULT_MAPPED_CRITERIONS_CSV = (
    str(REPO_ROOT / "evaluation/sequential/01_operationalization/outputs/mapped_criterions.csv")
)

DEFAULT_CLUSTER_JSON = (
    str(
        REPO_ROOT
        / "src/backend/utils/official/cluster_criterions/results/04_semantically_clustered_items.json"
    )
)
...
DEFAULT_OUTPUT_DIR = (
    str(
        REPO_ROOT
        / "src/backend/utils/agentic_core/others/initial_observation_model_assets/helpers/00_LLM_based_mapping_based_predictor_ranks"
    )
)
...
CRITERION_PREDICTOR_CSV = (
    str(
        REPO_ROOT
        / "src/backend/utils/official/ontology_mappings/CRITERION/predictor_to_criterion/results/gpt-5-nano/predictor_to_criterion_edges_long.csv"
    )
)
