...
def _find_repo_root() -> Path:
    ...
    for candidate in [here, *here.parents]:
        ...
        if has_eval and (candidate / "README.md").exists():
            return candidate
    raise RuntimeError("Could not locate repository root from apply_readiness_check.py")
...
_READINESS_LABEL_INDEX = {label: idx for idx, label in enumerate(_READINESS_LABEL_ORDER)}


def _analysis_execution_plan(recommended_tier: str, tier3_variant: Optional[str]) -> Dict[str, Any]:
    tier = str(recommended_tier or "Tier0_DescriptivesOnly")
    variant = str(tier3_variant) if tier3_variant is not None else None

    run_tv_gvar = tier == "Tier3_LaggedDynamicNetwork" and variant == "TIME_VARYING_gVAR"
    run_stationary_gvar = tier == "Tier3_LaggedDynamicNetwork" and variant in {"TIME_VARYING_gVAR", "STATIC_gVAR"}
    run_corr_baseline = tier in {
        "Tier1_CorrelationMatrix",
        "Tier2_ContemporaneousPartialCorrelation",
        "Tier3_LaggedDynamicNetwork",
    }
    run_descriptives_only = tier == "Tier0_DescriptivesOnly"

    if run_tv_gvar:
        analysis_set = "tier3_full_time_varying"
    elif run_stationary_gvar:
    ...
    return {
        ...
        "run_tv_gvar": run_tv_gvar,
        "run_stationary_gvar": run_stationary_gvar,
        "run_correlation_baseline": run_corr_baseline,
        "run_descriptives_only": run_descriptives_only,
        "full_time_varying_ready": run_tv_gvar,
        "can_compute_momentary_impact": bool(run_tv_gvar or run_stationary_gvar),
        "notes": [
            "FullyReady is reserved for cases where full TIME_VARYING_gVAR is executable.",
        ],
    }


def _cap_readiness_label_for_analysis(
    proposed_label: str,
    recommended_tier: str,
    tier3_variant: Optional[str],
) -> Tuple[str, Optional[str]]:
    tier = str(recommended_tier or "Tier0_DescriptivesOnly")
    variant = str(tier3_variant) if tier3_variant is not None else None
    label = str(proposed_label)
    if label not in _READINESS_LABEL_INDEX:
        return "NotReady", "Readiness label was invalid; set to NotReady."

    if tier == "Tier3_LaggedDynamicNetwork" and variant == "TIME_VARYING_gVAR":
        cap_label = "FullyReady"
    elif tier == "Tier3_LaggedDynamicNetwork" and variant == "STATIC_gVAR":
        cap_label = "Ready_High"
    elif tier == "Tier2_ContemporaneousPartialCorrelation":
        cap_label = "Ready_Low"
    elif tier == "Tier1_CorrelationMatrix":
        cap_label = "PartiallyReady_High"
    else:
        cap_label = "PartiallyReady_Low"

    if _READINESS_LABEL_INDEX[label] <= _READINESS_LABEL_INDEX[cap_label]:
        return label, None

    note = (
        f"Readiness label capped from {label} to {cap_label} to match executable analysis set "
        f"(tier={tier}, variant={variant})."
    )
    return cap_label, note
...
def _apply_llm_adjustments(report: Dict[str, Any], llm_out: Dict[str, Any]) -> Tuple[Dict[str, Any], List[str]]:
    ...
    final_label, cap_note = _cap_readiness_label_for_analysis(final_label, final_tier, final_variant)
    if cap_note:
    ...
    overall["analysis_execution_plan"] = _analysis_execution_plan(final_tier, final_variant)
...
def analyze_profile(
    ...
    if readiness_label_cap_note:
        tier_gate_notes.append(readiness_label_cap_note)
    analysis_execution_plan = _analysis_execution_plan(recommended_tier, tier3_variant)
    ...
    report: Dict[str, Any] = {
        ...
        "overall": {
            ...
            "analysis_execution_plan": analysis_execution_plan,
