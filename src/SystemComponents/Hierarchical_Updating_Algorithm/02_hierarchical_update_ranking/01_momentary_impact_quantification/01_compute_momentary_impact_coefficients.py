...
- Preferred source is tv-gVAR outputs; if unavailable, a stationary gVAR fallback is used.
- Local node metrics use temporal centrality when available; otherwise stationary centrality fallback is used.
- The script will raise AssertionError if neither compatible source is available, or if profile roles are invalid.

Inputs (from your network-analysis pipeline)
...
- network_metrics/temporal_lagged_node_centrality.csv (preferred)
  OR network_metrics/stationary_lagged_node_centrality.csv (fallback)
...
def load_profile(profile_dir: Path) -> ProfileData:
    ...
    elif st_npz_path.exists():
        impact_source = "stationary_gvar_fallback"
        st_npz = np.load(st_npz_path, allow_pickle=False)
        for k in ["B", "partial_corr"]:
            assert k in st_npz, f"[ASSERTION FAILED] {profile_id}: stationary NPZ missing '{k}' in {st_npz_path}"
        B = np.asarray(st_npz["B"], dtype=float)
        P = np.asarray(st_npz["partial_corr"], dtype=float)
        assert B.ndim == 2 and B.shape[0] == B.shape[1], (
            f"[ASSERTION FAILED] {profile_id}: stationary B expected (p,p), got shape {B.shape}"
        )
        assert P.ndim == 2 and P.shape == B.shape, (
            f"[ASSERTION FAILED] {profile_id}: stationary partial_corr shape mismatch. partial_corr={P.shape}, B={B.shape}"
        )
    ...
    assert Bhat.shape[1] == len(var_codes), (
        f"[ASSERTION FAILED] {profile_id}: matrix variable dimension {Bhat.shape[1]} does not match "
        f"selected var count {len(var_codes)} from {sel_path}"
    )
    ...
    if (pred_importance_tv is None or pred_importance_tv.empty) and impact_source == "stationary_gvar_fallback":
        pi_path_st = profile_dir / "network_metrics/predictor_importance_stationary.csv"
        pred_importance_tv = safe_read_csv(pi_path_st) if pi_path_st.exists() else pred_importance_tv
    ...
    # LOCAL METRICS: prefer temporal; fallback to stationary centrality if temporal is absent/skipped.
    nc_path = profile_dir / OUTPUT_SPECS["temporal_lagged_node_centrality"]["relpath"]
    temporal_node_centrality = safe_read_csv(nc_path) if nc_path.exists() else pd.DataFrame()
    ...
    assert not temporal_node_centrality.empty, (
        ...
        f"Expected temporal metrics at {profile_dir / OUTPUT_SPECS['temporal_lagged_node_centrality']['relpath']} "
        f"or stationary fallback at {profile_dir / 'network_metrics/stationary_lagged_node_centrality.csv'}"
    )
...
def build_payload(
    ...
    return {
        ...
        "meta": {
            ...
            "impact_source": data.impact_source,
...
def run_one_profile(profile_dir: Path, out_root: Path, half_life: float, top_k_edges: int) -> None:
    ...
    log(f"Impact source: {data.impact_source}")
...
def main() -> int:
    ...
    for d in prof_dirs:
        try:
            ...
            msg = str(e)
            if ("missing both tv and stationary arrays" in msg) or ("<2 vars" in msg):
                n_skip += 1
                log(f"[SKIP] {d.name}: {msg}")
            else:
                n_fail += 1
                log(msg)
    ...
    write_json(
        out_root / "run_summary.json",
        {
            ...
            "n_profiles_total": len(prof_dirs),
            "n_profiles_success": n_ok,
            "n_profiles_skipped": n_skip,
            "n_profiles_failed": n_fail,
        },
    )
